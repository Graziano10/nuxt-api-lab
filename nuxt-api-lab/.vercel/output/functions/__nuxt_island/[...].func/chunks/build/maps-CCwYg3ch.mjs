...
const _sfc_main$1 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    const canSearch = computed(() => {
      return Boolean(city.value.trim() || (lat.value.trim() && lon.value.trim()))
    })
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(
        `<form${ssrRenderAttrs(mergeProps({ class: 'space-y-4' }, _attrs))}><div class="grid grid-cols-1 md:grid-cols-4 gap-2">`
      )
      _push(
        ssrRenderComponent(
          BaseInput,
      ...
      _push(
        `</div><div class="relative"><div id="map" class="h-[500px] w-full rounded-card shadow-card"></div><div class="absolute top-4 right-4 z-[1000] flex gap-2 rounded-card bg-surface p-2 shadow-card">`
      )
      _push(
        ssrRenderComponent(
          BaseButton,
          ...
          },
          {
            default: withCtx((_, _push2, _parent2, _scopeId) => {
              if (_push2) {
                ...
                return [createTextVNode(' Satellite ')]
              }
            }),
            _: 1
          },
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    return (_ctx, _push, _parent, _attrs) => {
      _push(
        `<div${ssrRenderAttrs(mergeProps({ class: 'p-6 space-y-4' }, _attrs))}><h1 class="text-3xl font-bold">Mappe interattive</h1><p class="text-gray-600 max-w-2xl"> In questa pagina integriamo una mappa stile Google Earth usando OpenStreetMap, Leaflet e tiles satellitari ESRI. </p>`
      )
