...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    const averageTemperature = computed(() => {
      ...
      const sum = data.value.reduce((acc, item) => acc + item.temp.average, 0)
      return (sum / data.value.length).toFixed(1)
    })
    return (_ctx, _push, _parent, _attrs) => {
      _push(
        `<div${ssrRenderAttrs(mergeProps({ class: 'p-8 space-y-6' }, _attrs))}><h1 class="text-3xl font-bold"> 🌱 Agriculture Dashboard </h1>`
      )
      if (unref(pending)) {
