  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (!props.loading) {
        ...
        _push(
          `<span class="flex items-center gap-2"><span class="h-4 w-4 animate-spin rounded-full border-2 border-white/30 border-t-white"></span>`
        )
        ssrRenderSlot(
          _ctx.$slots,
          'loading',
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props, { emit: __emit }) {
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (__props.label) {
        _push(
          `<label class="text-sm font-medium text-text-secondary">${ssrInterpolate(__props.label)}</label>`
        )
      } else {
        _push(`<!---->`)
      }
