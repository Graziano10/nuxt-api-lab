<script setup lang="ts">
  ...
  import { Search, MapPin, Thermometer, Droplets, Wind, Sunrise } from 'lucide-vue-next'
  ...
  const debounceCity = useDebounceFn((value: string) => {
...
<template>
  <main class="min-h-screen flex justify-center px-4 py-16">
    <BaseCard class="w-full max-w-md space-y-6">
      <!-- Header -->
      <header>
        <Typography variant="h1">Weather API Lab</Typography>

        <Typography variant="body-sm" muted class="mt-1">
      ...
      <form class="space-y-4" @submit.prevent="onSubmit">
        <BaseInput v-model="city" label="City" placeholder="e.g. Rome">
          <template #icon-left>
        ...
        <BaseButton type="submit" :loading="loading" :disabled="!canSubmit">
          <template #icon-left>
            <Search v-if="!loading" class="h-5 w-5" />
          </template>
      ...
      <!-- Skeleton -->
      <BaseCard v-if="loading" class="animate-pulse space-y-4">
      ...
      <!-- Result -->
      <BaseCard v-if="data && !loading" class="bg-surface-muted space-y-4">
        <div class="flex items-center gap-3">
          ...
          <div>
            ...
            <Typography variant="body-sm" muted class="capitalize">
              {{ data.condition }}
            </Typography>
          </div>

          <BaseButton size="sm" @click="unit = unit === 'metric' ? 'imperial' : 'metric'">
            {{ unit === 'metric' ? '°C' : '°F' }}
          </BaseButton>
        </div>
      ...
      <!-- Empty -->
      <Typography v-if="!data && !error && !loading" variant="caption" muted class="text-center">
